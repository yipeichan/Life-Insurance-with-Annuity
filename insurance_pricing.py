...
class file:
     ...
     def readMortality(sex):  #read data
             Location = r'/users/yi-peichan/desktop/Life-Insurance-with-Annuity/mortality.csv'
...
while True:
     ...
     for i in range(0,pt):
          if i>0:
               pricingVs[i]=pricingV[i]+termpB[i-1]+survB[i-1]  #pricing 保價含生存保險金 policy value with annuity amount included
